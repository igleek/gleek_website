...
const Gleekify = () => {
    ...
    const signatureText = 'GLEEKIFY\ngleek.lol 💦'; // Customize this message
    ...
    const addElementToCanvas = (url, x, y, type) => {
        ...
        img.onload = () => {
            ...
            if (type === 'gleek') {
                ...
                const maxMemeWidth = 599; // Maximum width for memes
                const maxMemeHeight = 599; // Maximum height for memes
    ...
    const addImageToCanvas = (src) => {
        ...
        img.onload = () => {
            ...
            // Add the image to the canvas here
            // You might need to scale the image to fit the adjusted canvas size if required
            setElements((prevElements) => [
                ...prevElements,
                {
                    src,
            ...
            textElements.map((textElement) => {
                if (textElement.id === selectedId) {
                    return {
                        ...textElement,
                        fontSize: Math.max(textElement.fontSize - 2, 1),
                    }; // Decrease font size by 2, minimum of 1
                }
                return textElement;
            })
        );
