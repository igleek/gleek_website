    ...
    useEffect(() => {
        ...
        app.current.ticker.add((delta) => {
            if (isGameRunning) {
                ...
                if (
                    npc.current.x >
                        app.current.screen.width - npc.current.width ||
                    npc.current.x < 0
                ) {
                ...
                if (level === 17) {
                    ...
                    if (nextNpcShootTime <= 0) {
                        ...
                        nextNpcShootTime =
                            Math.random() +
                            Math.max(
                                baseNpcShootFrequency - frequencyReduction,
                                50
                ...
                if (level === 17) {
                    if (
                        npc.current.y >
                        app.current.screen.height - npc.current.height
                    ) {
                        npc.current.y =
                            app.current.screen.height - npc.current.height;
                        npcVy = 0;
        ...
        <div>
            <div className="game-wrapper">
                ...
                <div className="game-top-level">
                    ...
                    {displayLevel && (
                        <div className="level-display">{displayLevel}</div>
                    )}
                    {isGameRunning && (
